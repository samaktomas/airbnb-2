...
function ImageUpload({ onChange, value }: Props) {
  ...
  return (
    <CldUploadWidget
      ...
      //uploadPreset: cloudinary account settings -> Upload -> Upload presets -> Signing mode -> Unsigned!!
      uploadPreset="sobysp9a"
      options={{
        maxFiles: 1,
      }}
    >
          ...
          >
            ...
            {value && (
              <div className="absolute inset-0 w-full h-full">
                <Image fill alt="Upload" src={value} className="object-cover" />
              </div>
            )}
          </div>
        );
